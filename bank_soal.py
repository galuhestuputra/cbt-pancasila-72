...
def halaman_ujian():
    ...
    if st.session_state.submit_status == "Berhasil":
        ...
        for i, q in enumerate(soal_cbt):
            ...
            # Menampilkan opsi dengan warna penanda khusus
            for opsi in st.session_state.opsi_acak_soal.get(q["id"], q["opsi"]):
                if opsi == kunci_benar:
                    # Jika opsi adalah Kunci Jawaban yang Benar -> Beri warna hijau tebal
                    ...
                    # Jika opsi dipilih siswa tapi salah -> Beri warna merah tebal
